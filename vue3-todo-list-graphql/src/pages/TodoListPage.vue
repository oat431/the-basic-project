...
import TodoListGQLService from '../services/TodoListGQLService';
import TaskGQLService from '../services/TaskGQLService';
...
const createTask = handleSubmit(async (value: { [x: string]: string }, {resetForm}) => {
    Nprogress.start();
    await TaskGQLService.createTask(props.id, value.name);
    const gql = await TodoListGQLService.getTodoListById(props.id);
    todoList.data = gql.data.data.getTodoListById;
    resetForm();
    Nprogress.done();
});
...
const deleteTodoList = async () => {
    Nprogress.start();
    await TodoListGQLService.deleteTodoList(props.id);
    Nprogress.done();
...
const updateTodoList = async (value: {[x: string]: string}) => {
    Nprogress.start();
    await TodoListGQLService.updateTodoList(props.id, value.title, value.description);
    const gql = await TodoListGQLService.getTodoListById(props.id);
    todoList.data = gql.data.data.getTodoListById;
    editMark.value = !editMark.value;
    Nprogress.done();
...
<template>
    ...
    <DefaultLayout>
        ...
        <form ref="form" class="p-3" :validation-schema="schema" @submit="createTask" >
            <div class="mt-2">
                <p>
