...
onMounted(async () => {
    const gql = await TodoListGQLService.getTodoList(1, 5);
    data.value = gql.data.data.getTodoList.content;
    limit.value = gql.data.data.getTodoList.totalPages;
});
...
const nextPage = async () => {
    ...
    const gql = await TodoListGQLService.getTodoList(page.value+1, 5);
    data.value = gql.data.data.getTodoList.content;
    Nprogress.done();
}
...
const createTodoList = async (value : {[x:string]:string}) => {
    Nprogress.start();
    await TodoListGQLService.createTodoList(value.title, value.description);
    Nprogress.done();
    router.go(0);
}
