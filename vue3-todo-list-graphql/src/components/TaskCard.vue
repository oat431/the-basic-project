...
const deleteTask = async () => {
    nProgress.start();
    await TaskGQLService.deleteTask(props.id);
    const response = await TodoListGQLService.getTodoListById(props.todoId);
    todoList.data = response.data.data.getTodoListById;
    nProgress.done();
}

const updateStatus = async () => {
    nProgress.start();
    await TaskGQLService.updateTaskStatus(props.id);
    const response = await TodoListGQLService.getTodoListById(props.todoId);
    todoList.data = response.data.data.getTodoListById;
    nProgress.done();
}
...
const updateTask = async () => {
    nProgress.start();
    await TaskGQLService.updateTask(props.id, showName.value);
    const response = await TodoListGQLService.getTodoListById(props.todoId);
    todoList.data = response.data.data.getTodoListById;
    editMark.value = !editMark.value;
    nProgress.done();
